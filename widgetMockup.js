		...
		if (items[i].title === title) return items[i];
			...
			$(document).ready(function () {
				...
				$.each(danielHaggerty.consultr_items, function (index, value) {
					...
					$newItem.popover({ animation: true, html: true, trigger: 'hover', content: "<div class='consultrPopover'>" + value.short_description + "</div>", delay: {show: 1000, hide: 0}, placement: 'right'});
				...
				$('.consultrBtn').click(function () {
					...
					$('#consultrModalLabel').html(currItem.title);
					$('#consultrModalBody').html(currItem.long_description);
					$('#consultrModal').modal();
				})
			});
